...
class _Builder:
    ...
    def add(
        self, name, span_type, parent, inputs, outputs, duration_ms, status="OK", status_message=""
    ) -> str:
...
def build(entropy: int) -> tuple[dict, dict]:
    ...
    b.history.append(
        "[system] 你是一个数据分析助手。必须严格依据工具返回的数据回答，"
    ...
    for round_no in range(1, ROUNDS + 1):
        turn = b.add(
            f"turn_{round_no}", "CHAIN", root, {"round": round_no}, {"round": round_no}, 3_500
        )
        prompt = "\n".join(b.history)
        completion = (
            f"第 {round_no} 轮：我需要先查询营收数据，再按业务线拆分。计划调用工具并整理结果。"
        )
        model = b.add(
            "gpt-4o",
            "LLM",
        ...
        for i in range(TOOLS_PER_ROUND):
            ...
            b.add(
                ...
                {"results": [{"url": f"https://example.com/{i}", "content": page}]},
                340,
            )
            if i == 0:
    ...
    doc = {
        "info": {"request_id": "tr-blind-0001", "trace_id": _TRACE_ID, "execution_time_ms": 48_200},
        "data": {"spans": b.spans},
    }
    key = {
        "archetype": arch["key"],
